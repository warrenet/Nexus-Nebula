...
export function formatPrometheusMetrics(): string {
  ...
  lines.push(
    "# HELP nexus_red_team_flags_total Total number of red team flags triggered",
  );
  ...
  if (metrics.requestDurations.length > 0) {
    ...
    lines.push(
      `nexus_request_duration_seconds{quantile="0.5"} ${(p50 / 1000).toFixed(3)}`,
    );
    lines.push(
      `nexus_request_duration_seconds{quantile="0.9"} ${(p90 / 1000).toFixed(3)}`,
    );
    lines.push(
      `nexus_request_duration_seconds{quantile="0.99"} ${(p99 / 1000).toFixed(3)}`,
    );
    lines.push(
      `nexus_request_duration_seconds_count ${metrics.requestDurations.length}`,
    );
    lines.push(
      `nexus_request_duration_seconds_sum ${(metrics.requestDurations.reduce((a, b) => a + b, 0) / 1000).toFixed(3)}`,
    );
  }
