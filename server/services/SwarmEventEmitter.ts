...
export interface SwarmEvent {
    traceId: string;
    type: 'agent_start' | 'agent_thought' | 'agent_complete' | 'critique_start' | 'critique_complete' | 'synthesis_start' | 'synthesis_complete' | 'consensus_update';
...
class SwarmEventEmitterClass extends EventEmitter {
    ...
    subscribeToTrace(traceId: string, callback: (event: SwarmEvent) => void): () => void {
        const handler = (event: SwarmEvent) => callback(event);
        this.on(`swarm:${traceId}`, handler);
        return () => this.off(`swarm:${traceId}`, handler);
    }

    subscribeToThoughts(traceId: string, callback: (thought: AgentThought) => void): () => void {
