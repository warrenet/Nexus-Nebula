...
export function classifyInput(input: string): TieringResult {
  ...
  const missionIndicatorCount = MISSION_INDICATORS.filter((indicator) =>
  ...
  if (
    wordCount >= MIN_MISSION_WORD_COUNT ||
    charCount >= MIN_MISSION_CHAR_COUNT
  ) {
    return {
      tier: "mission",
      confidence: 0.75,
 ...
 */
export function getTierCostEstimate(tier: "task" | "mission"): {
  min: number;
  max: number;
} {
  if (tier === "task") {
