...
export async function cacheMission(
    ...
    if (Platform.OS === 'web') {
        // Web fallback
        cacheToLocalStorage({ id, mission, result, timestamp: Date.now(), cost, consensusScore });
        return;
    }
 ...
 */
export async function getCachedMission(id: string): Promise<CachedMission | null> {
    if (Platform.OS === 'web') {
 ...
 */
export async function searchCachedMissions(keyword: string): Promise<CachedMission[]> {
    if (Platform.OS === 'web') {
        const missions = getFromLocalStorage();
...
export async function getCacheStats(): Promise<{
    ...
    if (Platform.OS === 'web') {
        ...
        return {
            ...
            avgConsensus: missions.reduce((sum, m) => sum + m.consensusScore, 0) / missions.length,
    ...
    return result || { count: 0, totalCost: 0, avgConsensus: 0, oldestTimestamp: null };
}
