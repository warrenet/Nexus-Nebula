 ...
 */
export function DecisionTreeViewer({ trace, onAgentSelect }: DecisionTreeViewerProps) {
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        trace.iterations.forEach((iteration, iterIdx) => {
            ...
            const sourceId = iterIdx === 0 ? 'mission' : `iter-${trace.iterations[iterIdx - 1].iterationId}`;
            edges.push({
            ...
            iteration.agentResponses.forEach((agent, agentIdx) => {
                ...
                const confidenceColor = agent.confidence > 0.8 ? '#10B981' :
                    agent.confidence > 0.5 ? '#F59E0B' : '#EF4444';
    ...
    return (
        <div style={{ width: '100%', height: '600px', background: '#0A0A0F', borderRadius: '12px' }}>
            <ReactFlow
                nodes={nodesState}
                edges={edgesState}
            ...
            >
                ...
                {selectedAgent && (
                    <Panel position="top-right" style={{
                        ...
                        <p style={{ fontSize: '12px', color: '#06B6D4', margin: '0 0 8px 0' }}>
                            Agent: {selectedAgent.agentId} | Confidence: {(selectedAgent.confidence * 100).toFixed(1)}%
                        </p>
                        <div style={{
                            fontSize: '13px',
                            lineHeight: 1.5,
